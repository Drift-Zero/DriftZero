...
export function HealthChart({data,height=230,showMetrics=false}:{data:HealthPoint[];height?:number;showMetrics?:boolean}){
  ...
  const renderDot=useMemo(()=>{
    function HealthDot({cx,cy,index}:DotProps){
      ...
      if(!sparse)return <g/>
      return <circle cx={cx} cy={cy} r={2.5} fill="#131826" stroke="#38e8b8" strokeWidth={1.5}/>
    }
    return HealthDot
  },[lastIndex,sparse])
  ...
  return <div className={`health-chart${reducedMotion?' still':''}`} style={{height}}><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{top:12,right:RIGHT_GUTTER,bottom:4,left:-22}}>
    <defs><linearGradient id="healthFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#38e8b8" stopOpacity={.28}/><stop offset="100%" stopColor="#38e8b8" stopOpacity={0}/></linearGradient></defs>
    <CartesianGrid stroke="#1f2536" strokeDasharray="2 5" vertical={false}/>
    <XAxis dataKey="time" tickFormatter={shortTime} tick={{fill:'#5c6377',fontSize:10,fontFamily:'Inter'}} axisLine={false} tickLine={false}/>
    <YAxis domain={[0,100]} ticks={[25,50,75,100]} tick={{fill:'#5c6377',fontSize:10,fontFamily:'Inter'}} axisLine={false} tickLine={false}/>
    <Tooltip content={<ChartTooltip/>}/>
    <ReferenceLine y={WARNING_SCORE} stroke="#ffbe5c" strokeDasharray="4 4"/>
    ...
    <Line type="monotone" dataKey="score" name="Health" stroke="#38e8b8" strokeWidth={2.5} dot={renderDot} isAnimationActive={animate} animationDuration={DRAW_MS} animationEasing="ease-out" activeDot={{r:4,fill:'#131826',stroke:'#38e8b8',strokeWidth:2}}/>
    {showMetrics&&<Line type="monotone" dataKey="groundedness" name="Groundedness" stroke="#7c9bff" strokeWidth={1.5} dot={false} isAnimationActive={animate} animationDuration={DRAW_MS} animationBegin={140} animationEasing="ease-out"/>}
    {showMetrics&&<Line type="monotone" dataKey="stability" name="Stability" stroke="#c689ff" strokeWidth={1.5} dot={false} isAnimationActive={animate} animationDuration={DRAW_MS} animationBegin={280} animationEasing="ease-out"/>}
