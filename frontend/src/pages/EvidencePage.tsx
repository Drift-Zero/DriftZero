...
export function EvidencePage() {
  ...
  const [websiteUseGroq, setWebsiteUseGroq] = useState(false)
  ...
  return <div className="page evidence-page">
    ...
    <section className="panel website-monitor">
      <div className="panel-heading"><div><p className="panel-label">Live source / Website evidence</p><h2>Keep reference data current</h2><p>DriftZero reads JSON and GeoJSON directly. For webpages, Groq can optionally extract source-backed facts before scheduled checks.</p></div><Globe2 size={18}/></div>
      <form className="website-monitor-form" onSubmit={connectWebsite}>
        ...
        <label className="website-option"><input type="checkbox" checked={websiteUseGroq} onChange={event => setWebsiteUseGroq(event.target.checked)}/><span><strong>Structure webpages with Groq</strong><small>Uses the backend-only Groq key. JSON feeds remain deterministic.</small></span></label>
